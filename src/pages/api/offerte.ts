...
function adminOfferteHtml(data: {
  ...
  return baseLayout(`
    ...
    <h1 style="margin:0 0 24px;color:#111111;font-size:26px;font-weight:700;">Offerte Aanvraag</h1>
    ...
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td><a href="mailto:${data.email}" style="display:inline-block;background:#111;color:#fff;font-size:13px;font-weight:700;text-decoration:none;padding:12px 24px;border-radius:4px;">Reageer op aanvraag</a></td>
...
function klantOfferteHtml(naam: string): string {
  ...
  return baseLayout(`
    ...
    <p style="margin:0 0 24px;color:#555;font-size:15px;line-height:1.7;">Uw offerte-aanvraag is in goede orde ontvangen. Wij nemen <strong style="color:#111;">binnen 24 uur</strong> contact met u op voor een vrijblijvende offerte op maat.</p>
    <div style="background:#111;border-radius:6px;padding:24px 28px;margin-bottom:28px;">
      <p style="margin:0 0 14px;color:#fff;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;">Wat kunt u verwachten?</p>
    ...
    <div style="border:1px solid #e0e0e0;border-radius:6px;padding:20px 24px;margin-bottom:28px;">
      <p style="margin:0 0 12px;color:#111;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Heeft u een spoedeisende vraag?</p>
    ...
    <p style="margin:0;color:#999;font-size:12px;line-height:1.6;">U ontvangt dit bericht omdat u een offerte-aanvraag heeft ingediend via projectbouw010.nl.</p>
  `);
}
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    // Collect file attachments (base64)
    ...
    const adminResult = await sendEmail(apiKey, adminPayload);
    if (!adminResult.ok) {
